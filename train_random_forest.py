"""
==========================================================================
 WhaleGuard — Phase 4b: Random Forest Model Training & Evaluation
==========================================================================
 Trains a Random Forest classifier on the fully-engineered NARW habitat
 dataset using a temporally-ordered train/test split.
 ...
 Design Decisions:
   ...
   - class_weight="balanced_subsample": Adjusts class weights within each
     bootstrap sample, compensating the 1:4 pseudo-absence imbalance while
     respecting the bagging procedure (Gowan & Ortega-Ortiz, 2014).
   - Features EXCLUDE Lat, Lon, Date: Forces the model to learn ocean
     physics and seasonality, not memorise patrol coordinates.
   - Imputation (median): Random Forest in scikit-learn cannot handle NaN
     natively; median imputation is robust to outliers (unlike mean).
   - 500 Trees: Matches XGBoost's ensemble size for a fair comparison.

 Outputs:
   - Terminal classification report (Accuracy, Precision, Recall, F1, AUC)
   - images/rf_roc_curve.png
   - images/rf_precision_recall_tradeoff.png
   - images/rf_feature_importance.png
   - models/rf_narw_sdm.joblib  (serialised model for deployment)
==========================================================================
"""
...
RF_PARAMS = {
    "n_estimators":     500,           # same ensemble size as XGBoost for fair comparison
    "max_depth":        20,            # deeper than XGBoost (6) because RF controls variance via bagging
    "min_samples_split": 10,           # prevents overly specific splits
    "min_samples_leaf":  5,            # ensures minimum leaf population
    "max_features":     "sqrt",        # classic RF: sqrt(n_features) per split
    "class_weight":     "balanced_subsample",  # per-tree class rebalancing
...
# Plot style (matching EDA dark theme from other scripts)
plt.rcParams.update({
...
def main():
    t_start = time.time()
    print_header("WhaleGuard — Random Forest NARW Habitat Model")
    ...
    base_pipeline = Pipeline([
        ...
        ("clf",     RandomForestClassifier(
            ...
            bootstrap=True,
            oob_score=False,  # disable during CV (not needed)
            n_jobs=-1,
            random_state=42,
        )),
    ])
    ...
    for param, value in sorted(best_params.items()):
        print(f"    {param.replace('clf__', ''):20s} = {value}")

    # Retrain final model with best params + OOB scoring
    print_section("4b. Final Training (Best Parameters + OOB)")
    ...
    # Fill under curve
    ax.fill_between(fpr_curve, tpr_curve, alpha=0.15, color="#f0883e")
    ax.plot(fpr_curve, tpr_curve, color="#f0883e", linewidth=2.5,
    ...
    ax.set_title("ROC Curve — Random Forest NARW Habitat Model\n"
    ...
    fig.tight_layout()
    roc_path = IMG_DIR / "rf_roc_curve.png"
    ...
    ax.plot(rec_curve, prec_curve, color="#f0883e", linewidth=2.5,
            label="Precision-Recall Curve")
    ax.fill_between(rec_curve, prec_curve, alpha=0.1, color="#f0883e")
    ...
    ax.set_title("Precision-Recall Tradeoff — Random Forest NARW Habitat Model\n"
    ...
    fig.tight_layout()
    pr_path = IMG_DIR / "rf_precision_recall_tradeoff.png"
    ...
    # Also compute standard deviation across trees for error bars
    ...
    colors = plt.cm.YlOrRd(np.linspace(0.25, 0.85, len(sorted_idx)))
    bars = ax.barh(
    ...
    for bar, val in zip(bars, importances[sorted_idx]):
        ax.text(bar.get_width() + importances_std[sorted_idx[0]] + 0.005,
    ...
    fig.tight_layout()
    fi_path = IMG_DIR / "rf_feature_importance.png"
    ...
    for rank, idx in enumerate(reversed(sorted_idx)):
        ...
        print(f"    {rank+1}. {feature_names[idx]:20s}  {importances[idx]:.4f} ± {importances_std[idx]:.4f}  {bar}")

    # ── 8. Save Model ────────────────────────────────────────────────
    print_section("8. Saving Model & Optimal Threshold")

    model_path = MODEL_DIR / "rf_narw_sdm.joblib"
    joblib.dump(pipeline, str(model_path))
    print(f"  ✓ Model saved: {model_path}")
    print(f"  ✓ Model format: joblib Pipeline (imputer + RF classifier)")
    print(f"  ✓ To load: pipeline = joblib.load('{model_path}')")

    # Save the optimal threshold alongside the model
    threshold_path = MODEL_DIR / "rf_optimal_threshold.txt"
    with open(threshold_path, "w") as f:
        f.write(f"# NARW Habitat Model — Random Forest Optimal Classification Threshold\n")
